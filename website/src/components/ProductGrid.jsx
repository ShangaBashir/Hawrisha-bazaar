...
export default function ProductGrid({ onAddToCart, onCategorySelect, wishlist = [], onToggleWishlist }) {
  ...
  const getLocalized = (value, lang) => {
    if (!value) return '';
    if (typeof value === 'object') return value[lang] || value.en || value.ku || value.ar || '';
    if (typeof value === 'string') {
      if (value.startsWith('{')) {
        try {
          const parsed = JSON.parse(value);
          return parsed[lang] || parsed.en || parsed.ku || parsed.ar || value;
        } catch (e) {}
      }
      return value;
    }
  ...
  return (
    <section className="w-full bg-[#f4f3e6] py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
