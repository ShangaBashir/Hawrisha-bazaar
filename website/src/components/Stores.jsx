...
export default function Stores({ cart, likedProducts, onAddToCart, onRemoveFromCart, onToggleWishlist, onProductClick, isLoggedIn, onLoginRequired, resetTrigger, initialSearchTerm, initialStoreId }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 font-sans text-brand-charcoal" dir={isRTL ? 'rtl' : 'ltr'}>
      <AnimatePresence mode="wait">
        {!selectedStore ? (
          ...
          >
            ...
            {/* ── MOBILE ONLY: Store name + description above banner ── */}
            <div className="sm:hidden px-1 space-y-2">
              <h1 className="text-base font-black text-[#36454F] uppercase tracking-wide leading-tight">
                    ...
                    {Object.entries(links).map(([platform, url]) => {
                      ...
                      return (
                        <a
                          ...
                          className="w-7 h-7 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-600 transition-all flex items-center justify-center shadow-2xs"
                          title={platform}
                        >
            ...
            {/* ── DESKTOP/TABLET ONLY: Store name + description + social links above banner ── */}
            <div className="hidden sm:flex flex-row justify-between items-start w-full px-2 pb-2">
              <div className="space-y-2 max-w-3xl text-start">
                <h1 className="text-2xl lg:text-3xl font-black text-[#36454F] uppercase tracking-wider leading-none">
                  {parseEn(selectedStore.name)}
                </h1>
              ...
              {/* Social links top-right for desktop */}
              {(() => {
                const parseSocialLinks = (val) => {
                  if (!val) return {};
                  try { return JSON.parse(val); } catch { return {}; }
                    ...
                    {Object.entries(links).map(([platform, url]) => {
                      ...
                      return (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="w-8 h-8 rounded-full bg-white border border-gray-200/80 hover:bg-gray-50 hover:border-gray-300 text-[#36454F] transition-all flex items-center justify-center shadow-2xs hover:scale-105"
                          title={platform}
                        >
                          {platform === 'facebook' && <FacebookIcon size={13} />}
                          {platform === 'instagram' && <InstagramIcon size={13} />}
                          {platform === 'twitter' && <TwitterIcon size={13} />}
                          {platform === 'website' && <GlobeIcon size={13} />}
                        </a>
                      );
                    })}
                  </div>
            ...
            {/* Storefront Banner */}
            <div className="w-full sm:h-80 lg:h-96 rounded-3xl overflow-hidden relative border border-gray-100 bg-gray-50 mb-10 sm:mb-12 sm:flex sm:items-center sm:justify-center flex flex-col">
              
              {/* Cover Banner Image */}
              <div className="w-full h-48 sm:h-full relative bg-gray-50 flex items-center justify-center">
                <img src={selectedStore.banner ? (selectedStore.banner.startsWith('/') ? selectedStore.banner : `/uploads/${selectedStore.banner}`) : '/categories/cat2.jpg'} 
                  ...
                  className="w-full h-full object-cover brightness-95 sm:brightness-100 relative block select-none pointer-events-none z-2" onError={(e) => { e.target.onerror = null; e.target.src = '/categories/cat1.jpg'; }} />

                {/* Desktop overlay: Logo bottom-left inside banner */}
                <div className="absolute bottom-6 start-6 z-10 hidden sm:block">
                  {selectedStore.logo ? (
                    <img src={selectedStore.logo.startsWith('/') ? selectedStore.logo : `/uploads/${selectedStore.logo}`} 
                      alt={selectedStore.name} 
                      className="w-24 h-24 rounded-2xl object-cover border-4 border-white shadow-lg bg-white" onError={(e) => { e.target.onerror = null; e.target.src = '/categories/cat1.jpg'; }} />
                  ) : (
                    <div className="w-24 h-24 rounded-2xl bg-white text-[#B2AC88] flex items-center justify-center border-4 border-white shadow-lg">
                      <Store size={38} />
                    </div>
                  )}
                </div>

                {/* Desktop overlay: City Tag floating top-right inside banner */}
                {selectedStore.city && (
                  <div className="hidden sm:flex absolute top-6 right-6 z-10 items-center gap-1.5 text-white text-xs font-bold uppercase tracking-wider drop-shadow-md bg-black/20 backdrop-blur-xs px-3.5 py-1.5 rounded-full border border-white/20">
                    <MapPin size={12} className="text-white" />
                    <span>{parseEn(selectedStore.city)}</span>
                  </div>
                )}

                {/* Mobile: City Tag floating top-right inside banner */}
                {selectedStore.city && (
                  <div className="absolute top-4 right-4 z-10 flex sm:hidden items-center gap-1 text-white text-[10px] font-bold uppercase tracking-wider drop-shadow-md bg-black/40 backdrop-blur-xs px-2.5 py-1.5 rounded-full border border-white/20">
                    <MapPin size={10} className="text-white" />
                    <span>{parseEn(selectedStore.city)}</span>
                  </div>
                )}

                {/* Mobile: Logo pinned bottom-left of banner */}
                <div className="absolute bottom-3 left-3 z-10 sm:hidden">
                  {selectedStore.logo ? (
                    <img src={selectedStore.logo.startsWith('/') ? selectedStore.logo : `/uploads/${selectedStore.logo}`}
                      alt={selectedStore.name}
                      className="w-14 h-14 rounded-2xl object-cover border-2 border-white shadow-lg bg-white" onError={(e) => { e.target.onerror = null; e.target.src = '/categories/cat1.jpg'; }} />
                  ) : (
                    <div className="w-14 h-14 rounded-2xl bg-white text-[#B2AC88] flex items-center justify-center border-2 border-white shadow-lg">
                      <Store size={24} />
                    </div>
                  )}
                </div>
            ...
            {/* ── DESKTOP/TABLET ONLY: Product Count directly below banner ── */}
            <div className="hidden sm:flex flex-col px-1 mt-4 mb-2 text-start">
              <h2 className="text-xl lg:text-2xl font-black text-[#36454F] uppercase tracking-wider mb-1">
                {language === 'ar' ? 'جميع المنتجات' : language === 'ku' ? 'هەموو بەرهەمەکان' : 'ALL PRODUCTS'}
              </h2>
            ...
            {/* ── MOBILE ONLY: product count directly below banner ── */}
            <div className="sm:hidden flex flex-col px-1 mt-4 mb-2 text-start">
              <h2 className="text-base font-black text-[#36454F] uppercase tracking-wider mb-1">
                {language === 'ar' ? 'جميع المنتجات' : language === 'ku' ? 'هەموو بەرهەمەکان' : 'ALL PRODUCTS'}
              </h2>
