...
export default function Account({ email, onBackToHome, onLogoutClick, onViewChange, onSelectStore }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 font-sans text-brand-charcoal" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-10 pb-6 border-b border-gray-200/60">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-[#36454F] uppercase tracking-wider">
            {t('account_page.title')}
          </h1>
          <p className="text-2xl sm:text-3xl text-[#B2AC88] font-black mt-1 tracking-wide">
                              ...
                              <div>
                                ...
                                {order.items && order.items[0] ? (
                                  <button
                                    type="button"
                                    onClick={() => {
                                      const storeId = order.items[0]?.store_id;
                                      if (storeId && onSelectStore) {
